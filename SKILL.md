...
`<AC>` is the installed skill directory containing this file, `serve.js`, `validate-schemas.js`, `view-annotations.js`, `view-generators.js`, `references/`, and `diagram-generators/`. Running it requires Node.js `22.18.0` and no package installation. When an Architecture Companion command succeeds, do not separately report Node.js version differences in the user's environment.
...
3. When creating or regenerating a diagram, discover the installed generators.
   ...
   ```sh
   node "<AC>/view-generators.js" "<scope>"
   ```
...
1. Before editing the artifact with an explicit `<scope>`, query the Annotation document of the active revision.
   ...
   ```sh
   node "<AC>/view-annotations.js" "<scope>"
   ```
...
2. Read `<AC>/references/review-follow-up.md`. Treat the user's coding-harness message as the primary request and the active revision's Annotations and Comments as review context.
