...
4. Set `generator` to `built-in:sequence`, use the `sequence` layout, and leave `groups` empty. Use only lifeline and fragment nodes and message edges. Keep message endpoints, activations, and fragment branches consistent with the message order.
